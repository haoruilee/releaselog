...
- **Subscribe:** `/subscribe` — Atom feeds at `/feeds/atom.xml` and `/feeds/{entityId}/atom.xml` (readers poll; entries link back to the calendar). Optional weekly email: set `UPSTASH_REDIS_REST_URL`, `UPSTASH_REDIS_REST_TOKEN`, `RESEND_API_KEY`, `DIGEST_FROM_EMAIL`, `CRON_SECRET`; trigger `/api/cron/weekly-digest` with `Authorization: Bearer CRON_SECRET` from a scheduler (self-hosted: see `deploy/releaselog-weekly-digest.timer`).
...
- **Admin + ingest:** `/admin/candidates`, `/admin/releases`, `/admin/subscribers`. Release notifications are enqueued at approval and delivered asynchronously by the `/api/cron/send-notifications` worker (paired timer `deploy/releaselog-send.timer`). Ingestion cron is `/api/cron/ingest` (paired timer `deploy/releaselog-ingest.timer`).
